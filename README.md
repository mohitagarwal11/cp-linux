...
A minimal reimplementation of the Unix `cp` command using low-level POSIX system calls (`open`, `read`, `write`).
...
* Does not preserve source file permissions
...
* Use `stat()` and `chmod()` to preserve permissions
